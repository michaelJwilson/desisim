...
Quickly and easily generate galaxy spectra.
"""
...
def parse(options=None):
    parser=argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                   description="""Fast simulation of galaxy spectra into the final DESI format
...
def simulate_one_healpix(ifilename, args, model, obsconditions, decam_and_wise_filters,
    ...
    if args.dla is not None :
        ...
        if len(dla_id)>0:    
            ...
            hdla = fits.convenience.table_to_hdu(dla_meta); hdla.name="DLA_META"
            hdlalist = fits.HDUList([fits.PrimaryHDU(),hdla])
    ...
    if args.zbest is not None :
        ...
        hzbest = fits.convenience.table_to_hdu(zbest); hzbest.name="ZBEST"
        hfmap  = fits.convenience.table_to_hdu(fibermap);  hfmap.name="FIBERMAP"

        hdulist =fits.HDUList([fits.PrimaryHDU(),hzbest,hfmap])
...
def main(args=None):
    ...
    if args.desi_footprint :
        ...
        if not os.path.isfile(footprint_filename): 
            ...
            sys.exit(1)
        pixmap=fits.open(footprint_filename)[0].data
